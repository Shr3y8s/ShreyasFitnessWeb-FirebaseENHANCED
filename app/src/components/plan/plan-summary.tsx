      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-2 flex-wrap">
          <CardTitle className="flex items-center gap-3 text-lg sm:text-xl min-w-0">
              <Pin className="w-6 h-6 shrink-0 text-green-600" />
              <span>This Week&apos;s Focus</span>
          </CardTitle>
              ...
              <InfoSection title="Adjustments Made" icon={<TrendingUp className="h-5 w-5 text-green-600" />}>
                  <ul className="space-y-2">
                      {currentWeekData.adjustments.map((item, index) => (
                          /* items-start + shrink-0 dot: this list previously used
                             items-center with a non-shrink dot, so a long
                             adjustment squashed the marker. Now matches Priorities. */
                          <li key={index} className="flex items-start gap-3 p-3 bg-green-100/60 dark:bg-green-900/20 rounded-md">
                              <div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-green-600" />
                              <span className="text-sm font-medium text-green-700 dark:text-green-300 min-w-0">{item}</span>
                          </li>
                      ))}
                  </ul>
              </InfoSection>
              ...
              <InfoSection title="Priorities" icon={<Check className="h-5 w-5 text-green-600" />}>
                   <ul className="space-y-2">
                      {currentWeekData.priorities.map((item, index) => (
                          <li key={index} className="flex items-start gap-3 p-3 bg-background rounded-md">
                             <div className="mt-1.5 h-2 w-2 rounded-full bg-green-600 flex-shrink-0" />
                             <span className="text-sm font-medium text-foreground min-w-0">{item}</span>
                         </li>
                     ))}
                  </ul>
              </InfoSection>
